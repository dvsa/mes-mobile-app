...
Then('I should see the Microsoft login page', () => {
  ...
  browser.driver.getCurrentContext().then((webviewContext) => {
    ...
    browser.driver.selectContext('NATIVE_APP').then(() => {
      ...
      // Switch back to WEBVIEW context
      browser.driver.selectContext(Page.getParentContext(webviewContext));
    });
  });
});
...
Then('I should see the {string} contains {string}', (rowName, rowValue) => {
  const dataRow = Page.getAndAwaitElement(by.xpath(`//ion-col/label[text()= "${rowName}"]
...
When('I click on the {string} button', (buttonId) => {
  Page.clickButtonByCssId(buttonId);
});

Then('validation item {string} should be visible', (validationId: string) => {
  const validationElement = Page.getElementByCssId(validationId);
...
Then(/^the (communication page|waiting room|debrief|health declaration) candidate name should be "(.+)"$/, (
  ...
  const candidateNameElement = Page.getCandidateNameElement(pageName, this.testCategory);
...
Then(/^the (communication page|waiting room|debrief|health declaration) candidate driver number should be "(.+)"$/, (
  ...
  const candidateDriverNumberElement = Page.getCandidateDriveNumberElement(pageName, this.testCategory);
