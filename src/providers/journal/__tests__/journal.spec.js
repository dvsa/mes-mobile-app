...
describe('JournalProvider', function () {
    var journalProvider;
    var httpMock;
    ...
    describe('isCacheTooOld', function () {
        it("should return true if date is greater than cacheDays days ago", function () {
            var today = new DateTime();
            var tooOld = new DateTime().add(-(cacheDays + 1), Duration.DAY);
            expect(journalProvider.isCacheTooOld(tooOld, today)).toBe(true);
        });
        it("should return true if date is less than or equal to cacheDays days ago", function () {
            var today = new DateTime();
    ...
    describe('getAndConvertOfflineJournal', function () {
        it('should empty cached data if cache is too old', function () {
            var exampleSchedule = {
            ...
            journalProvider.getAndConvertOfflineJournal().then(function (data) {
                expect(journalProvider.emptyCachedData).toHaveBeenCalled();
                expect(dataStoreMock.setItem).toHaveBeenCalled();
                expect(data).toEqual({});
            });
        });
        it('should return data without emptying cache if data is not too old', function () {
            var exampleSchedule = {
                examiner: { staffNumber: '1234' },
            };
    ...
    describe('getJournal', function () {
        it('should obtain the personal journal URL from the journal provider, passing the cached employee ID', function () {
