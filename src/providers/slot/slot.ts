...
export class SlotProvider {
  ...
  extendWithEmptyDays = (slots: {[k: string]: SlotItem[]}): {[k: string]: SlotItem[]} => {
    ...
    const days = times(numberOfDaysToView, (d: number): string => now().add(d, Duration.DAY).format('YYYY-MM-DD'));
  ...
  getSlotDate = (slot: any): string => at(slot.slotData.slotDetail.start).format('YYYY-MM-DD');
