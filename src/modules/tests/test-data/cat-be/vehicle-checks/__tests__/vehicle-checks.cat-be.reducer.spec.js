import { initialState, vehicleChecksCatBEReducer, } from '../vehicle-checks.cat-be.reducer';
import { ShowMeQuestionSelected, ShowMeQuestionOutcomeChanged, TellMeQuestionSelected, TellMeQuestionOutcomeChanged, AddShowMeTellMeComment, } from '../vehicle-checks.cat-be.action';
import { cloneDeep } from 'lodash';
describe('Vehicle Checks Cat B+E Reducer', function () {
    describe('SHOW_ME_QUESTION_SELECTED', function () {
        it('should add the show me question in the payload to the array at the specified index', function () {
            ...
            var result = vehicleChecksCatBEReducer(state, new ShowMeQuestionSelected(newQuestionPayload, 1));
    ...
    describe('SHOW_ME_QUESTION_OUTCOME_CHANGED', function () {
        it('should update the outcome property for the object at the specified index', function () {
            ...
            var result = vehicleChecksCatBEReducer(state, new ShowMeQuestionOutcomeChanged('DF', 1));
    ...
    describe('TELL_ME_QUESTION_SELECTED', function () {
        it('should add the tell me question in the payload to the array at the specified index', function () {
            ...
            var state = cloneDeep(initialState);
            var result = vehicleChecksCatBEReducer(state, new TellMeQuestionSelected(newQuestionPayload, 1));
    ...
    describe('TELL_ME_QUESTION_OUTCOME_CHANGED', function () {
        it('should update the outcome property for the object at the specified index', function () {
            ...
            var result = vehicleChecksCatBEReducer(state, new TellMeQuestionOutcomeChanged('DF', 1));
    ...
    describe('ADD_SHOW_ME_TELL_ME_COMMENT', function () {
        it('should update the vehicle checks comments', function () {
            var state = cloneDeep(initialState);
            var result = vehicleChecksCatBEReducer(state, new AddShowMeTellMeComment('So many mistakes.'));
            expect(result.showMeTellMeComments).toEqual('So many mistakes.');
        });
    });
});
//# sourceMappingURL=vehicle-checks.cat-be.reducer.spec.js.map