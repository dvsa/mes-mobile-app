...
import { AddShowMeTellMeComment, } from '../../../modules/tests/test-data/cat-be/vehicle-checks/vehicle-checks.cat-be.action';
...
import { CAT_BE, DELEGATED_REKEY_UPLOAD_OUTCOME_PAGE, JOURNAL_PAGE, } from '../../page-names.constants';
...
import { vehicleChecksExist, } from '../../../modules/tests/test-data/cat-be/vehicle-checks/vehicle-checks.cat-be.selector';
...
var OfficeCatBEPage = /** @class */ (function (_super) {
    __extends(OfficeCatBEPage, _super);
    function OfficeCatBEPage(store$, toastController, navController, navParams, platform, authenticationProvider, weatherConditionProvider, questionProvider, keyboard, outcomeBehaviourProvider, alertController, faultCountProvider, faultSummaryProvider, appConfig) {
        ...
        _this.shouldDisplayDrivingFaultComments = function (data) {
            var drivingFaultCount = _this.faultCountProvider.getDrivingFaultSumCount("B+E" /* BE */, data);
            var seriousFaultCount = _this.faultCountProvider.getSeriousFaultSumCount("B+E" /* BE */, data);
            var dangerousFaultCount = _this.faultCountProvider.getDangerousFaultSumCount("B+E" /* BE */, data);
    ...
    OfficeCatBEPage.prototype.ionViewDidEnter = function () {
        this.store$.dispatch(new OfficeViewDidEnter());
    };
    OfficeCatBEPage.prototype.ngOnInit = function () {
        var _this = this;
        ...
        this.pageState = {
            ...
            displayDrivingFault$: currentTest$.pipe(select(getTestOutcome), withLatestFrom(currentTest$.pipe(select(getTestData))), map(function (_a) {
                ...
                return _this.outcomeBehaviourProvider.isVisible(outcome, 'faultComment', _this.faultSummaryProvider.getDrivingFaultsList(data, "B+E" /* BE */));
            })),
            displaySeriousFault$: currentTest$.pipe(select(getTestOutcome), withLatestFrom(currentTest$.pipe(select(getTestData))), map(function (_a) {
                var outcome = _a[0], data = _a[1];
                return _this.outcomeBehaviourProvider.isVisible(outcome, 'faultComment', _this.faultSummaryProvider.getSeriousFaultsList(data, "B+E" /* BE */));
            })),
            ...
            dangerousFaults$: currentTest$.pipe(select(getTestData), map(function (data) { return _this.faultSummaryProvider.getDangerousFaultsList(data, "B+E" /* BE */); })),
            seriousFaults$: currentTest$.pipe(select(getTestData), map(function (data) { return _this.faultSummaryProvider.getSeriousFaultsList(data, "B+E" /* BE */); })),
            drivingFaults$: currentTest$.pipe(select(getTestData), map(function (data) { return _this.faultSummaryProvider.getDrivingFaultsList(data, "B+E" /* BE */); })),
            drivingFaultCount$: currentTest$.pipe(select(getTestData), map(function (data) { return _this.faultCountProvider.getDrivingFaultSumCount("B+E" /* BE */, data); })),
            seriousFaultCount$: currentTest$.pipe(select(getTestData), map(function (data) { return _this.faultCountProvider.getSeriousFaultSumCount("B+E" /* BE */, data); })),
            dangerousFaultCount$: currentTest$.pipe(select(getTestData), map(function (data) { return _this.faultCountProvider.getDangerousFaultSumCount("B+E" /* BE */, data); })),
    ...
    OfficeCatBEPage.prototype.setupSubscription = function () {
        var _this = this;
        var _a = this.pageState, transmission$ = _a.transmission$, testOutcome$ = _a.testOutcome$, delegatedTest$ = _a.delegatedTest$, testOutcomeText$ = _a.testOutcomeText$, conductedLanguage$ = _a.conductedLanguage$, startDateTime$ = _a.startDateTime$;
        this.subscription = merge(conductedLanguage$.pipe(map(function (value) { return _this.conductedLanguage = value; })), delegatedTest$.pipe(map(function (value) { return _this.isDelegated = value; })), transmission$.pipe(map(function (value) { return _this.transmission = value; })), testOutcome$.pipe(map(function (value) { return _this.testOutcome = value; })), testOutcomeText$.pipe(map(function (value) { return _this.testOutcomeText = value; })), startDateTime$.pipe(map(function (value) { return _this.startDateTime = value; }))).subscribe();
    };
    OfficeCatBEPage.prototype.ionViewDidLeave = function () {
        if (this.subscription) {
            this.subscription.unsubscribe();
        }
    };
    OfficeCatBEPage.prototype.popToRoot = function () {
    ...
    OfficeCatBEPage.prototype.defer = function () {
        this.popToRoot();
        this.store$.dispatch(new SavingWriteUpForLater());
        this.store$.dispatch(new PersistTests());
    };
    OfficeCatBEPage.prototype.onSubmit = function () {
        if (this.isFormValid()) {
            this.showFinishTestModal();
        }
    };
    OfficeCatBEPage.prototype.setIsValidStartDateTime = function (isValid) {
        this.isValidStartDateTime = isValid;
    };
    OfficeCatBEPage.prototype.dateOfTestChanged = function (inputDate) {
    ...
    OfficeCatBEPage.prototype.identificationChanged = function (identification) {
        this.store$.dispatch(new IdentificationUsedChanged(identification));
    };
    OfficeCatBEPage.prototype.independentDrivingChanged = function (independentDriving) {
        this.store$.dispatch(new IndependentDrivingTypeChanged(independentDriving));
    };
    OfficeCatBEPage.prototype.weatherConditionsChanged = function (weatherConditions) {
        this.store$.dispatch(new WeatherConditionsChanged(weatherConditions));
    };
    OfficeCatBEPage.prototype.routeNumberChanged = function (routeNumber) {
        this.store$.dispatch(new RouteNumberChanged(routeNumber));
    };
    OfficeCatBEPage.prototype.candidateDescriptionChanged = function (candidateDescription) {
        this.store$.dispatch(new CandidateDescriptionChanged(candidateDescription));
    };
    OfficeCatBEPage.prototype.additionalInformationChanged = function (additionalInformation) {
        this.store$.dispatch(new AdditionalInformationChanged(additionalInformation));
    };
    OfficeCatBEPage.prototype.dangerousFaultCommentChanged = function (dangerousFaultComment) {
    ...
    OfficeCatBEPage.prototype.seriousFaultCommentChanged = function (seriousFaultComment) {
    ...
    OfficeCatBEPage.prototype.drivingFaultCommentChanged = function (drivingFaultComment) {
    ...
    OfficeCatBEPage.prototype.activityCodeChanged = function (activityCodeModel) {
    ...
    OfficeCatBEPage.prototype.showFinishTestModal = function () {
        var _this = this;
    ...
    OfficeCatBEPage.prototype.goToReasonForRekey = function () {
        if (this.isFormValid()) {
            this.navController.push(CAT_BE.REKEY_REASON_PAGE);
        }
    };
    OfficeCatBEPage.prototype.isFormValid = function () {
        var _this = this;
    ...
    OfficeCatBEPage.prototype.completeTest = function () {
        if (!this.isDelegated) {
    ...
    OfficeCatBEPage.prototype.provisionalLicenseReceived = function () {
        this.store$.dispatch(new ProvisionalLicenseReceived());
    };
    OfficeCatBEPage.prototype.provisionalLicenseNotReceived = function () {
        this.store$.dispatch(new ProvisionalLicenseNotReceived());
    };
    OfficeCatBEPage.prototype.transmissionChanged = function (transmission) {
        this.store$.dispatch(new GearboxCategoryChanged(transmission));
    };
    OfficeCatBEPage.prototype.healthDeclarationChanged = function (healthSigned) {
        this.store$.dispatch(new postTestDeclarationsActions.HealthDeclarationAccepted(healthSigned));
    };
    OfficeCatBEPage.prototype.passCertificateNumberChanged = function (passCertificateNumber) {
    ...
    OfficeCatBEPage.prototype.d255Changed = function (d255) {
        this.store$.dispatch(d255 ? new D255Yes() : new D255No());
    };
    OfficeCatBEPage.prototype.debriefWitnessedChanged = function (debriefWitnessed) {
        this.store$.dispatch(debriefWitnessed ? new DebriefWitnessed() : new DebriefUnwitnessed());
    };
    OfficeCatBEPage.prototype.isWelshChanged = function (isWelsh) {
    ...
    return OfficeCatBEPage;
}(BasePageComponent));
export { OfficeCatBEPage };
//# sourceMappingURL=office.cat-be.page.js.map