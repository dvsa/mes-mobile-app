import { ActivityCodes, ActivityCodesDelegatedExaminer } from '../../../../shared/models/activity-codes';
...
export enum ActivityCodeDescriptionDelegatedExaminer {
...
/**
 * Populate Activity codes.  If delegated examiner add additional codes to list
 * @param DLG
 */
export function populateActivityCodeModelList(DLG?: boolean): ActivityCodeModel[] {
  const codeList = [];
  ...
  if (DLG) {
    Object.keys(ActivityCodesDelegatedExaminer).forEach(code => codeList.push({
      activityCode: ActivityCodesDelegatedExaminer[code],
      description: ActivityCodeDescriptionDelegatedExaminer[code],
    }));
  }
  codeList.sort((a, b) => (parseInt(a.activityCode, 10) > parseInt(b.activityCode, 10)) ? 1 : -1);
  return codeList;
}
