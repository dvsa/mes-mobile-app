...
export class CommunicationCatDPage extends BasePageComponent implements OnInit {
  ...
  dispatchCandidateChoseProvidedEmail() {
    this.setCommunicationType(CommunicationCatDPage.email, CommunicationCatDPage.providedEmail);
    this.store$.dispatch(
      new CandidateChoseEmailAsCommunicationPreference(
        this.candidateProvidedEmail, CommunicationCatDPage.email),
    );
  }
  ...
  dispatchCandidateChoseNewEmail(communicationEmail: string): void {
    this.store$.dispatch(
      new CandidateChoseEmailAsCommunicationPreference(
        communicationEmail, CommunicationCatDPage.email),
    );
  }
  ...
  isProvidedEmailSelected() {
    return (this.communicationType === CommunicationCatDPage.email
      && this.emailType === CommunicationCatDPage.providedEmail);
  }

  isNewEmailSelected() {
    return (this.communicationType === CommunicationCatDPage.email
      && this.emailType === CommunicationCatDPage.updatedEmail);
  }

  isPostSelected() {
    return this.communicationType === CommunicationCatDPage.post;
  }

  dispatchCandidateChosePost(): void {
    this.setCommunicationType(CommunicationCatDPage.post);
    this.store$.dispatch(
      new CandidateChosePostAsCommunicationPreference(CommunicationCatDPage.post),
    );
  }
  ...
  restoreRadiosFromState() {
    if (this.communicationType === CommunicationCatDPage.email) {
  ...
  initialiseDefaultSelections() {
    this.communicationType = CommunicationCatDPage.email;
  ...
  verifyNewEmailFormControl(communicationChoice: string) {
    ...
    if (newEmailCtrl !== null) {
      if (communicationChoice !== CommunicationCatDPage.email
        || this.emailType === CommunicationCatDPage.providedEmail) {
  ...
  shouldPreselectADefaultValue(): boolean {
    return this.communicationType === CommunicationCatDPage.notProvided;
  }
  ...
  conditionalDispatchCandidateChoseNewEmail() {
    this.setCommunicationType(CommunicationCatDPage.email, CommunicationCatDPage.updatedEmail);
