...
export class CommunicationCatAMod1Page extends BasePageComponent implements OnInit {
    ...
    if (this.form.valid) {
      this.deviceAuthenticationProvider.triggerLockScreen()
        .then(() => {
          ...
          this.navController.push(CAT_A_MOD1.WAITING_ROOM_TO_CAR_PAGE)
            .then(() => {
              const waitingRoomPage =
              this.navController.getViews().find(view => view.id === CAT_A_MOD1.WAITING_ROOM_PAGE);
              if (waitingRoomPage) {
                this.navController.removeView(waitingRoomPage);
              }
              const communicationPage =
                this.navController.getViews().find(view => view.id === CAT_A_MOD1.COMMUNICATION_PAGE);
  ...
  dispatchCandidateChoseProvidedEmail() {
    this.setCommunicationType(CommunicationCatAMod1Page.email, CommunicationCatAMod1Page.providedEmail);
    this.store$.dispatch(
      new CandidateChoseEmailAsCommunicationPreference(
        this.candidateProvidedEmail, CommunicationCatAMod1Page.email),
    );
  }
  ...
  dispatchCandidateChoseNewEmail(communicationEmail: string): void {
    this.store$.dispatch(
      new CandidateChoseEmailAsCommunicationPreference(
        communicationEmail, CommunicationCatAMod1Page.email),
    );
  }
  ...
  isProvidedEmailSelected() {
    return (this.communicationType === CommunicationCatAMod1Page.email
      && this.emailType === CommunicationCatAMod1Page.providedEmail);
  }

  isNewEmailSelected() {
    return (this.communicationType === CommunicationCatAMod1Page.email
      && this.emailType === CommunicationCatAMod1Page.updatedEmail);
  }

  isPostSelected() {
    return this.communicationType === CommunicationCatAMod1Page.post;
  }

  dispatchCandidateChosePost(): void {
    this.setCommunicationType(CommunicationCatAMod1Page.post);
    this.store$.dispatch(
      new CandidateChosePostAsCommunicationPreference(CommunicationCatAMod1Page.post),
    );
  }
  ...
  restoreRadiosFromState() {
    if (this.communicationType === CommunicationCatAMod1Page.email) {
  ...
  initialiseDefaultSelections() {
    this.communicationType = CommunicationCatAMod1Page.email;
  ...
  verifyNewEmailFormControl(communicationChoice: string) {
    ...
    if (newEmailCtrl !== null) {
      if (communicationChoice !== CommunicationCatAMod1Page.email
        || this.emailType === CommunicationCatAMod1Page.providedEmail) {
  ...
  shouldPreselectADefaultValue(): boolean {
    return this.communicationType === CommunicationCatAMod1Page.notProvided;
  }
  ...
  conditionalDispatchCandidateChoseNewEmail() {
    this.setCommunicationType(CommunicationCatAMod1Page.email, CommunicationCatAMod1Page.updatedEmail);
