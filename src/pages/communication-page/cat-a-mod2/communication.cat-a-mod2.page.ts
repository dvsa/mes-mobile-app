...
export class CommunicationCatAMod2Page extends BasePageComponent implements OnInit {
    ...
    if (this.form.valid) {
      this.deviceAuthenticationProvider.triggerLockScreen()
        .then(() => {
          ...
          this.navController.push(CAT_A_MOD2.WAITING_ROOM_TO_CAR_PAGE)
            .then(() => {
              const waitingRoomPage =
              this.navController.getViews().find(view => view.id === CAT_A_MOD2.WAITING_ROOM_PAGE);
              if (waitingRoomPage) {
                this.navController.removeView(waitingRoomPage);
              }
              const communicationPage =
                this.navController.getViews().find(view => view.id === CAT_A_MOD2.COMMUNICATION_PAGE);
  ...
  dispatchCandidateChoseProvidedEmail() {
    this.setCommunicationType(CommunicationCatAMod2Page.email, CommunicationCatAMod2Page.providedEmail);
    this.store$.dispatch(
      new CandidateChoseEmailAsCommunicationPreference(
        this.candidateProvidedEmail, CommunicationCatAMod2Page.email),
    );
  }
  ...
  dispatchCandidateChoseNewEmail(communicationEmail: string): void {
    this.store$.dispatch(
      new CandidateChoseEmailAsCommunicationPreference(
        communicationEmail, CommunicationCatAMod2Page.email),
    );
  }
  ...
  isProvidedEmailSelected() {
    return (this.communicationType === CommunicationCatAMod2Page.email
      && this.emailType === CommunicationCatAMod2Page.providedEmail);
  }

  isNewEmailSelected() {
    return (this.communicationType === CommunicationCatAMod2Page.email
      && this.emailType === CommunicationCatAMod2Page.updatedEmail);
  }

  isPostSelected() {
    return this.communicationType === CommunicationCatAMod2Page.post;
  }

  dispatchCandidateChosePost(): void {
    this.setCommunicationType(CommunicationCatAMod2Page.post);
    this.store$.dispatch(
      new CandidateChosePostAsCommunicationPreference(CommunicationCatAMod2Page.post),
    );
  }
  ...
  restoreRadiosFromState() {
    if (this.communicationType === CommunicationCatAMod2Page.email) {
  ...
  initialiseDefaultSelections() {
    this.communicationType = CommunicationCatAMod2Page.email;
  ...
  verifyNewEmailFormControl(communicationChoice: string) {
    ...
    if (newEmailCtrl !== null) {
      if (communicationChoice !== CommunicationCatAMod2Page.email
        || this.emailType === CommunicationCatAMod2Page.providedEmail) {
  ...
  shouldPreselectADefaultValue(): boolean {
    return this.communicationType === CommunicationCatAMod2Page.notProvided;
  }
  ...
  conditionalDispatchCandidateChoseNewEmail() {
    this.setCommunicationType(CommunicationCatAMod2Page.email, CommunicationCatAMod2Page.updatedEmail);
