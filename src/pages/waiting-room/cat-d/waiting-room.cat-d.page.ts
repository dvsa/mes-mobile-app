...
export class WaitingRoomCatDPage extends BasePageComponent implements OnInit {
  ...
  merged$: Observable<boolean | string | CatDUniqueTypes.JournalData>;
  ...
  ngOnInit(): void {
    ...
    this.merged$ = merge(
      currentTest$.pipe(
        select(getJournalData),
        tap((journalData: CatDUniqueTypes.JournalData) => {
  ...
  isJournalDataInvalid = (journalData: CatDUniqueTypes.JournalData): boolean => {
