...
@IonicPage()
@Component({
  selector: '.health-declaration-cat-adi-part2-page',
  templateUrl: 'health-declaration.cat-adi-part2.page.html',
})
export class HealthDeclarationCatADIPart2Page extends BasePageComponent {
        ...
        this.navController.push(CAT_ADI_PART2.BACK_TO_OFFICE_PAGE).then(() => {
          this.navController.getViews().forEach((view) => {
            if (includes([
              ...
              CAT_ADI_PART2.HEALTH_DECLARATION_PAGE,
            ],
              view.id)) {
