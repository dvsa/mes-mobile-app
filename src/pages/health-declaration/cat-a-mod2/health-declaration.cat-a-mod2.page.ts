...
export class HealthDeclarationCatAMod2Page extends BasePageComponent {
  ...
  persistAndNavigate(resetLicenseProvided: boolean) {
    this.deviceAuthenticationProvider.triggerLockScreen()
      .then(() => {
        ...
        this.navController.push(CAT_A_MOD2.BACK_TO_OFFICE_PAGE).then(() => {
          this.navController.getViews().forEach((view) => {
            if (includes([
              CAT_A_MOD2.TEST_REPORT_PAGE,
              CAT_A_MOD2.DEBRIEF_PAGE,
              CAT_A_MOD2.PASS_FINALISATION_PAGE,
              CAT_A_MOD2.HEALTH_DECLARATION_PAGE,
            ],
              view.id)) {
