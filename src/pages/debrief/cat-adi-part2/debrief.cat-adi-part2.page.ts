...
export class DebriefCatADIPart2Page extends BasePageComponent {
  ...
  endDebrief(): void {
    ...
    this.navController.push(CAT_ADI_PART2.NON_PASS_FINALISATION_PAGE).then(() => {
      ...
      const debriefPage = this.navController.getViews().find(view => view.id === CAT_ADI_PART2.DEBRIEF_PAGE);
      if (debriefPage) {
