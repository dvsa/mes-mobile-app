...
import { now, at, Duration } from '../../common/date-time';
...
export const getLastRefreshedTime = (date: Date) => isNil(date) ? '--:--' : at(date).format('hh:mma');
...
export const canNavigateToNextDay = (journal: JournalModel): boolean => {
  const availableDays = getAvailableDays(journal);
  const nextDay = at(journal.selectedDate).add(1, Duration.DAY).format('YYYY-MM-DD');
  
  return availableDays.includes(nextDay);
};

export const isToday = (selectedDate: string): boolean => now().format('YYYY-MM-DD') === selectedDate;